---
name: sheet_navigator
description: >
  Navegador de Google Sheets por secciones. Busca el nombre de la tarjeta
  en las pestañas del Spreadsheet y el nombre del mes dentro de la hoja
  para filtrar los datos y devolverlos listos para renderizar en el portal.
---

# Skill: Sheet Navigator — Lectura por Tarjeta y Mes

## Objetivo
Leer el Google Sheet del sistema de Caja Menor y devolver los datos estructurados
para cada pantalla del portal: lista de tarjetas (Pantalla A), lista de meses
disponibles (Pantalla B) y filas de datos de un mes específico (Pantalla C).

---

## Función 1 — Obtener lista de tarjetas (`get_cards_list`)
...
def get_cards_list(service_sheet, sheet_id: str) -> list[dict]:
    ...
    Cada dict contiene:
      ...
      - 'cellphone'  : celular del empleado (de Employee model)
    """
    from core.services.google_sheets import get_visible_sheets_names, read_data
...
## Función 2 — Obtener meses disponibles en una hoja (`get_months_in_sheet`)

Los separadores de mes son filas con **fondo rojo** cuyo texto contiene el nombre del mes
(almacenado en la celda de la columna de fecha/centro costos de esa fila).
...
def get_months_in_sheet(service_sheet, sheet_id: str, sheet_name: str,
    ...
    """
    from core.services.google_sheets import read_data, get_cell_background_color
...
## Función 3 — Obtener filas de datos de un mes (`get_month_rows`)
...
def get_month_rows(service_sheet, sheet_id: str, sheet_name: str,
    ...
    Cada dict tiene:
      no, fecha, nombre_negocio, nit, num_factura, centro_costos, concepto,
      valor_total, valor_legalizado, url_drive, cufe, check_odoo_doc,
      check_odoo_pago, diferencia, observaciones, row_bg_color
    """
    from core.services.google_sheets import read_data, get_cell_background_color
...
## Identificación de meses (lógica clave)

Los separadores de mes tienen estas características en el Sheet:
1. **Fondo rojo** en todas las celdas de la fila (`red=1.0, green=0, blue=0`)
2. El texto del mes se escribe en la celda de la columna de fecha/centro costos con **texto en rojo** y `font_size=18`
3. El texto tiene formato: `"ENERO 2025"`, `"FEBRERO 2026"`, etc.

Para identificarlos se usa `get_cell_background_color()` ya implementada en `google_sheets.py`.

## Colores de filas de datos a preservar

| Color | Significado | Hex |
|-------|------------|-----|
| Blanco | Fila normal | `#FFFFFF` |
| Verde claro | Legalizado/aprobado | `#D9EAD3` aprox |
| Amarillo | Pendiente/atención | `#FFF2CC` aprox |

Se convierte el dict de color `{red, green, blue}` (0-1) a hex CSS para aplicar en el `style` de `<tr>`.
